...
async def rules(update):
    ...
    • После успешного подключения бота к беседе, каждый пользователь, желающий сыграть, должен написать в чат команду /play. Когда в игре есть хотя бы один человек - выбирается ведущий и начинается игра.
    • В случае, если ведущий вышел из игры или он не хочет пропустить ход - пропишите /skip.
    • Для выхода из игры нужно прописать /stop (даже если бот отключен и вы вступали в игру).
